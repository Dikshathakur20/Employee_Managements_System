...
useEffect(() => {
  if (employee) {
    ...
    setTimeout(() => setDesignationId(employee.designation_id?.toString() || ''), 0);
  }
}, [employee]);
...
const filteredDesignations = designations.filter(d => d.department_id === Number(departmentId));
...
try {
  ...
  await axiosClient.put(`/employees/${employee.employee_id}`, employeeData);
...
<div className="relative">
  ...
  onChange={(e) => {
    ...
    if (!isNaN(value) && value > 0 && value <= 10000000) setSalary(e.target.value);
