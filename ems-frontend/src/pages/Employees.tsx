...
const Employees = () => {
  ...
  // fetch employees, departments, designations and auth records (to mark registration)
 const fetchData = async () => {
  setLoading(true);
  try {
    const [employeesRes, deptsRes, desRes, authRes] = await Promise.all([
      axiosClient.get("/employees"),
      axiosClient.get("/departments"),
      axiosClient.get("/designations"),
      axiosClient.get("/auth").catch(() => ({ data: [] })),
    ]);

    const employeesData = employeesRes.data || [];
    const departmentsData = deptsRes.data || [];
    const designationsData = desRes.data || [];
    const authData = authRes?.data || [];

    const registeredIds = new Set(authData.map((a) => a.employee_id));
    ...
    toast({
      ...
      description: "Unable to fetch employee/department/designation data.",
      variant: "destructive",
    });
  } finally {
    setLoading(false);
  ...
  const getDesignationTitle = (designationId?: number | null) => {
    ...
    return designations.find((d) => d.designation_id === designationId)
      ?.designation_title || "Unknown";
  };

  // Add new employee handler (the NewEmployeeDialog should call the backend itself)
  const handleNewEmployee = (newEmp: Employee) => {
    // Prepend to list for instant feedback and set sort to show newest
  ...
  const handleDelete = async (employeeId: number) => {
    // confirm modal via toast with action buttons (keeps your existing UI pattern)
    toast({
      ...
      description: (
        <div className="flex justify-end gap-2 mt-2">
          ...
          <Button size="sm" variant="outline" className="bg-gray-300 text-black hover:bg-gray-400">
            Cancel
          </Button>
        </div>
      ),
  ...
  // Register employee (create auth record) — auto-generate password like you specified
  const handleRegister = async (emp: Employee) => {
    try {
      if (!emp.employee_code || !emp.phone || !emp.date_of_birth) {
        toast({ title: "Missing Data", description: "employee_code / phone / date_of_birth required." });
        return;
      }

      const last3Code = (emp.employee_code || "").slice(-3);
      const last4Phone = (emp.phone || "").slice(-4);
      const birthYear = emp.date_of_birth ? new Date(emp.date_of_birth).getFullYear() : "";
      ...
      toast({ title: "Registered Successfully", description: `Employee registered. Password: ${password}` });

      // Update local registration flag instantly
      setEmployees((prev) =>
        prev.map((e) => (e.employee_id === emp.employee_id ? { ...e, isRegistered: true } : e))
      );
    } catch (err: any) {
      console.error("Registration Error:", err);
      const msg = err?.response?.data?.message || "Could not register employee.";
  ...
  const renderProfilePicture = (emp?: Employee, size = 40) => {
    if (!emp) return <div className="rounded-full bg-gray-200 h-8 w-8 mx-auto" />;
    if (emp.file_data) {
      return (
        <div
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
              {(departmentFilter || designationFilter) && (
                <Button
                  ...
                  onClick={() => {
                    if (departmentFilter) navigate("/departments");
                    else if (designationFilter) navigate("/designations");
                  }}
                >
  ...
  {loading
    ...
    : // Actual data rows with selection
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mt-4 gap-2">
              ...
              <div className="flex items-center gap-2 ml-auto">
                ...
                <span className="px-2 py-1 rounded text-gray-700 bg-gray-200">
                  Page {currentPage} of {Math.max(1, Math.ceil(sortedEmployees.length / rowsPerPage))}
                </span>
                <Button
                  size="sm"
      ...
      {/* Dialogs (you already have these components in your project) */}
      <EditEmployeeDialog
          ...
          {viewingEmployee && (
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <div className="shrink-0">
                ...
                <div className="mt-4">
                  <Button size="sm" variant="outline" className="bg-blue-900 text-white hover:bg-blue-700 h-8 px-3" title="Edit Employee" onClick={() => { setEditingEmployee(viewingEmployee); setViewingEmployee(null); }}>
