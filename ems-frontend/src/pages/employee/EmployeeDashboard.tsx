...
// Use relative path from pages/employee
...
const fetchTasks = async (employeeId: number) => {
  try {
    const { data } = await axiosClient.get(`/tasks/employee/${employeeId}`); // same as TaskStatus
    setTasks(data || []);
  } catch (err) {
...
const fetchLeaves = async (employeeId: number) => {
  try {
    const { data } = await axiosClient.get(`/leaves/employee/${employeeId}`); // same as ApplyLeave
    setLeaves(data || []);
  } catch (err) {
...
const fetchNotifications = async () => {
  if (!employee) return; // make sure employee is loaded
  try {
    const { data } = await axiosClient.get("/notifications"); // fetch all notifications
...
try {
const today = new Date().toLocaleDateString("en-CA", { timeZone: "Asia/Kolkata" });
await axiosClient.put(`/attendance/${employee.id}`, { check_out: getISTDateTime(), date: today });
fetchAttendance(employee.id);
} catch (err) {
...
return (
<> <style>{marqueeStyle}</style> <div className="min-h-screen bg-gray-50">
{/* 🔷 Welcome Banner */} <div className="bg-[#001F7A] text-white py-3 overflow-hidden"> <div className="animate-marquee text-center text-lg font-semibold">
👋 Welcome back, {employee?.name || "Employee"} — Have a productive day! </div> </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              {(() => {
                ...
                if (!old) return <p className="text-gray-500 text-sm">No older notifications.</p>;
